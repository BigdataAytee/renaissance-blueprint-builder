// Curated AI-generated images per service card. Each image is picked to
// visually match the specific products, commodities, equipment or facilities
// mentioned in the corresponding sector-content service description.

// -------- Agriculture --------
import agriCrop1 from "@/assets/service-slides/agriculture/crop-1.jpg";
import agriCrop2 from "@/assets/service-slides/agriculture/crop-2.jpg";
import agriMech1 from "@/assets/service-slides/agriculture/mech-1.jpg";
import agriMech2 from "@/assets/service-slides/agriculture/mech-2.jpg";
import agriPalm1 from "@/assets/service-slides/agriculture/palm-1.jpg";
import agriPalm2 from "@/assets/service-slides/agriculture/palm-2.jpg";
import agriPoultry1 from "@/assets/service-slides/agriculture/poultry-1.jpg";
import agriPoultry2 from "@/assets/service-slides/agriculture/poultry-2.jpg";
import agriLivestock1 from "@/assets/service-slides/agriculture/livestock-1.jpg";
import agriLivestock2 from "@/assets/service-slides/agriculture/livestock-2.jpg";
import agriAgro1 from "@/assets/service-slides/agriculture/agro-1.jpg";
import agriAgro2 from "@/assets/service-slides/agriculture/agro-2.jpg";
import agriFert1 from "@/assets/service-slides/agriculture/fert-1.jpg";
import agriFert2 from "@/assets/service-slides/agriculture/fert-2.jpg";
import agriEquip1 from "@/assets/service-slides/agriculture/equip-1.jpg";
import agriEquip2 from "@/assets/service-slides/agriculture/equip-2.jpg";
import agriExport1 from "@/assets/service-slides/agriculture/export-1.jpg";
import agriExport2 from "@/assets/service-slides/agriculture/export-2.jpg";

// -------- Oil & Gas --------
import ogPetroleum from "@/assets/service-slides/oil-and-gas/petroleum-marketing.jpg";
import ogPetroleum2 from "@/assets/service-slides/oil-and-gas/petroleum-marketing-2.jpg";
import ogPetroleum3 from "@/assets/service-slides/oil-and-gas/petroleum-marketing-3.jpg";
import ogFuel from "@/assets/service-slides/oil-and-gas/fuel-distribution.jpg";
import ogFuel2 from "@/assets/service-slides/oil-and-gas/fuel-distribution-2.jpg";
import ogPipeline from "@/assets/service-slides/oil-and-gas/pipeline-epc.jpg";
import ogStorage from "@/assets/service-slides/oil-and-gas/storage-tanks.jpg";
import ogStorage2 from "@/assets/service-slides/oil-and-gas/storage-tanks-2.jpg";
import ogLng from "@/assets/service-slides/oil-and-gas/lng-supply.jpg";
import ogLpg from "@/assets/service-slides/oil-and-gas/lpg-distribution.jpg";
import ogLpg2 from "@/assets/service-slides/oil-and-gas/lpg-distribution-2.jpg";
import ogMaint from "@/assets/service-slides/oil-and-gas/maintenance-inspection.jpg";
import ogOffshore from "@/assets/service-slides/oil-and-gas/offshore-marine.jpg";
import ogSafety from "@/assets/service-slides/oil-and-gas/safety-systems.jpg";

// -------- Industrial Services --------
import indClean from "@/assets/service-slides/industrial-services/cleaning-sandblasting.jpg";
import indEquip from "@/assets/service-slides/industrial-services/equipment-installation.jpg";
import indEquip2 from "@/assets/service-slides/industrial-services/equipment-installation-2.jpg";
import indEquip3 from "@/assets/service-slides/industrial-services/equipment-installation-3.jpg";
import indCorr from "@/assets/service-slides/industrial-services/corrosion-coatings.jpg";
import indFab from "@/assets/service-slides/industrial-services/fabrication-welding.jpg";
import indPrev from "@/assets/service-slides/industrial-services/preventive-maintenance.jpg";
import indHeavy from "@/assets/service-slides/industrial-services/heavy-equipment.jpg";
import indHeavy2 from "@/assets/service-slides/industrial-services/heavy-equipment-2.jpg";

// -------- Logistics & Distribution --------
import logiFleet from "@/assets/service-slides/logistics-distribution/fleet-management.jpg";
import logiTruck from "@/assets/service-slides/logistics-distribution/trucking-haulage.jpg";
import logiWare from "@/assets/service-slides/logistics-distribution/warehousing.jpg";
import logiSupply from "@/assets/service-slides/logistics-distribution/supply-chain.jpg";
import logiCold from "@/assets/service-slides/logistics-distribution/cold-chain.jpg";
import logiFreight from "@/assets/service-slides/logistics-distribution/freight-forwarding.jpg";
import logiContainer from "@/assets/service-slides/logistics-distribution/container-transport.jpg";
import logiDist from "@/assets/service-slides/logistics-distribution/distribution-networks.jpg";
import logiLast from "@/assets/service-slides/logistics-distribution/last-mile.jpg";

// -------- Manufacturing & Trading --------
import mtMfg from "@/assets/service-slides/manufacturing-trading/industrial-manufacturing.jpg";
import mtWhole from "@/assets/service-slides/manufacturing-trading/wholesale-trading.jpg";
import mtRetail from "@/assets/service-slides/manufacturing-trading/retail-distribution.jpg";
import mtConsumer from "@/assets/service-slides/manufacturing-trading/consumer-products.jpg";
import mtIndProd from "@/assets/service-slides/manufacturing-trading/industrial-products.jpg";
import mtBuild from "@/assets/service-slides/manufacturing-trading/building-materials.jpg";
import mtProc from "@/assets/service-slides/manufacturing-trading/procurement.jpg";
import mtSupply from "@/assets/service-slides/manufacturing-trading/supply-chain.jpg";

// -------- Project & Property Management --------
import ppmPlan from "@/assets/service-slides/project-property-management/planning.jpg";
import ppmConstr from "@/assets/service-slides/project-property-management/construction-management.jpg";
import ppmCivil from "@/assets/service-slides/project-property-management/civil-engineering.jpg";
import ppmProp from "@/assets/service-slides/project-property-management/property-development.jpg";
import ppmFac from "@/assets/service-slides/project-property-management/facility-management.jpg";
import ppmArch from "@/assets/service-slides/project-property-management/architecture.jpg";
import ppmGen from "@/assets/service-slides/project-property-management/general-contracting.jpg";
import ppmMaint from "@/assets/service-slides/project-property-management/maintenance.jpg";

// -------- Hospitality & Entertainment --------
import hospHotel from "@/assets/service-slides/hospitality-entertainment/hotels.jpg";
import hospRest from "@/assets/service-slides/hospitality-entertainment/restaurants.jpg";
import hospCater from "@/assets/service-slides/hospitality-entertainment/catering.jpg";
import hospEvents from "@/assets/service-slides/hospitality-entertainment/events-weddings.jpg";
import hospConf from "@/assets/service-slides/hospitality-entertainment/conference-centres.jpg";
import hospFast from "@/assets/service-slides/hospitality-entertainment/fast-food.jpg";
import hospRec from "@/assets/service-slides/hospitality-entertainment/recreation-tourism.jpg";
import hospBev from "@/assets/service-slides/hospitality-entertainment/beverages.jpg";

// -------- Business Consultancy --------
import bcStrat from "@/assets/service-slides/business-consultancy/strategy.jpg";
import bcFin from "@/assets/service-slides/business-consultancy/financial.jpg";
import bcCorp from "@/assets/service-slides/business-consultancy/corporate-advisory.jpg";
import bcMkt from "@/assets/service-slides/business-consultancy/marketing.jpg";
import bcProj from "@/assets/service-slides/business-consultancy/project-advisory.jpg";
import bcTrain from "@/assets/service-slides/business-consultancy/training.jpg";
import bcProc from "@/assets/service-slides/business-consultancy/procurement.jpg";
import bcDig from "@/assets/service-slides/business-consultancy/digital-transformation.jpg";

export const serviceImages: Record<string, Record<string, string[]>> = {
  agriculture: {
    "Commercial Crop Production": [agriCrop1, agriCrop2],
    "Mechanised & Irrigation Farming": [agriMech1, agriMech2],
    "Palm Oil, Rubber & Ginger Value Chains": [agriPalm1, agriPalm2],
    "Poultry & Fish Farming": [agriPoultry1, agriPoultry2],
    Livestock: [agriLivestock1, agriLivestock2],
    "Agro-Processing, Storage & Packaging": [agriAgro1, agriAgro2],
    "Fertilizers, Seeds & Agrochemicals": [agriFert1, agriFert2],
    "Agricultural Equipment": [agriEquip1, agriEquip2],
    "Commodity Export & Advisory": [agriExport1, agriExport2],
  },
  "oil-and-gas": {
    "Petroleum Marketing": [ogPetroleum],
    "Fuel Distribution": [ogFuel],
    "Pipeline & EPC Support": [ogPipeline],
    "Storage Tanks & Terminals": [ogStorage],
    "LNG & Industrial Gas Supply": [ogLng],
    "LPG Distribution": [ogLpg],
    "Maintenance & Inspection": [ogMaint],
    "Offshore & Marine Logistics": [ogOffshore],
    "Safety Systems & Industrial Energy": [ogSafety],
  },
  "industrial-services": {
    "Industrial Cleaning & Sandblasting": [indClean],
    "Equipment Installation & Supply": [indEquip],
    "Corrosion Protection & Coatings": [indCorr],
    "Fabrication, Welding & Mechanical Repairs": [indFab],
    "Preventive Maintenance & Shutdown Services": [indPrev],
    "Heavy Equipment Servicing": [indHeavy],
  },
  "logistics-distribution": {
    "Fleet Management": [logiFleet],
    "Trucking & Haulage": [logiTruck],
    "Warehousing & Inventory Management": [logiWare],
    "Supply Chain Management": [logiSupply],
    "Cold Chain Logistics": [logiCold],
    "Freight Forwarding & Customs Clearance": [logiFreight],
    "Container Transport & Cargo Handling": [logiContainer],
    "Distribution Networks": [logiDist],
    "Last-Mile Delivery": [logiLast],
  },
  "manufacturing-trading": {
    "Industrial Manufacturing": [mtMfg],
    "Wholesale Trading": [mtWhole],
    "Retail Distribution": [mtRetail],
    "Consumer Products": [mtConsumer],
    "Industrial Products & Spare Parts": [mtIndProd],
    "Building Materials Distribution": [mtBuild],
    "Procurement & Sourcing": [mtProc],
    "Supply Chain": [mtSupply],
  },
  "project-property-management": {
    "Project Planning & Feasibility": [ppmPlan],
    "Construction Management": [ppmConstr],
    "Civil Engineering & Infrastructure": [ppmCivil],
    "Property Development & Estate Management": [ppmProp],
    "Facility Management": [ppmFac],
    "Architectural Design & Engineering Consultancy": [ppmArch],
    "General Contracting & Renovation": [ppmGen],
    "Maintenance Services": [ppmMaint],
  },
  "hospitality-entertainment": {
    "Hotels & Resorts": [hospHotel],
    Restaurants: [hospRest],
    "Catering & Banqueting": [hospCater],
    "Corporate Events & Wedding Venues": [hospEvents],
    "Conference Centres": [hospConf],
    "Fast Food & QSR": [hospFast],
    "Recreation & Tourism Services": [hospRec],
    Beverages: [hospBev],
  },
  "business-consultancy": {
    "Business Strategy & Feasibility Studies": [bcStrat],
    "Financial & Investment Advisory": [bcFin],
    "Corporate Advisory & Risk Management": [bcCorp],
    "Marketing & Market Research": [bcMkt],
    "Project Advisory & Business Development": [bcProj],
    "Corporate Training & HR Consulting": [bcTrain],
    "Procurement Consulting & Operations Support": [bcProc],
    "Digital & Business Transformation": [bcDig],
  },
};
