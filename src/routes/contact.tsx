...
import { submitContactMessage } from "@/lib/cms/public-forms.functions";
  ...
  head: () => ({
    meta: [
      { title: "Contact — Dynamic Renaissance" },
      {
        name: "description",
        content:
          "Get in touch with our team for consultations, partnerships and general enquiries.",
      },
...
function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setSubmitting(true);
    try {
      // The server verifies the captcha, applies the per-IP limit, writes the
      // row and sends the notification. The browser has no write access to
      // contact_messages any more, so this is the only route in.
      ...
      toast.error(errorMessage(err, "Could not send your message. Please try again."));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Layout>
      <PageHero
        ...
        subtitle="Request a consultation, partnership discussion or general enquiry — we respond within one business day."
      />
      <section className="section-y">
        <div className="container-wide grid gap-12 lg:grid-cols-[1fr_1.4fr]">
          <div className="space-y-6">
            {[
              ...
              <div key={c.t} className="p-6 rounded-lg border border-border flex gap-4">
                <div className="size-11 rounded-md bg-primary text-primary-foreground grid place-items-center shrink-0">
          ...
          <form
            ...
            className="relative overflow-hidden rounded-2xl bg-card border border-border p-8 md:p-10 lg:p-12 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.08)]"
          >
            ...
            <div className="mt-8 grid gap-6 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                ...
                <select
                  ...
                  className="mt-2 w-full cursor-pointer rounded-xl border border-border bg-muted px-4 py-3.5 text-sm text-foreground outline-none transition-all duration-300 hover:bg-background hover:border-primary/30 focus:border-primary focus:bg-background focus:ring-4 focus:ring-primary/10"
                >
  ...
  return (
    <div>
      ...
      <input
        ...
        type={type}
        required={required}
        className="mt-2 w-full rounded-xl border border-border bg-muted px-4 py-3.5 text-sm text-foreground placeholder:text-muted-foreground/60 outline-none transition-all duration-300 hover:bg-background hover:border-primary/30 focus:border-primary focus:bg-background focus:ring-4 focus:ring-primary/10"
      />
    </div>
  );
}
